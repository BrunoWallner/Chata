...
fn main() {
    ...
    loop {
        ...
        if window == 1 {
            ...
            login_window.write(Position::new(17, 20), login_status.clone(), Color::green());

            terminal.write_window(&login_window);
            terminal.render();

            ip = terminal.read_line(Position::new(login_window.pos.x + 5, login_window.pos.y + 3), "iP: ", Color::red(), true);
            ...
            if name.len() >= 1 {
                ...
                token = match check_login(ip.clone(), name, password) {
                    Ok(token) => {
                        //logged_in = true;
                        login_status = "logged in".to_string();
                        window = 2;
                        token
                    }
                    Err(e) => {
        ...
        if window == 2 {
            ...
            match terminal.key_event.code {
                ...
                KeyCode::Enter => {
                    ...
                    write_message(ip.clone(), input_string.clone(), token.clone());
                    input.clear();
                }
                KeyCode::Backspace => {
                    input.pop();
...
fn check_login(ip: String, name: String, password: String) -> Result<[u8; 256], String> {
...
fn write_message(ip: String, message: String, token: [u8; 256]) -> Result<(), String> {
    match TcpStream::connect(ip + ":" + PORT) {
        Ok(mut stream) => {
            let msg = create_message(message.clone(), token.clone());
...
fn create_message(message: String, token: [u8; 256]) -> [u8; 8192] {
    let mut buffer = [0x00 as u8; 8192];
    buffer[0] = 0x05;
